...
var count = 0;
var timelineTime;
var video1Q = document.getElementById('video1Q');
var video_1 = document.getElementById('video_1');
var video2Q = document.getElementById('video_2');
...
video1Q.addEventListener('play', function() {
    trackPlayProgress();
}, false);
...
function updatePlayProgress(){
    playProgressBar.style.width = ((video1Q.currentTime / video1Q.duration) * (progressHolder.offsetWidth - 2)) + "px";
    updateTimeDisplay();
}
...
function setPlayProgress(clickX) {
    ...
    video1Q.currentTime = newPercent * video1Q.duration;
    video_1.currentTime = newPercent * video_1.duration;
    video2Q.currentTime = newPercent * video2Q.duration;
    playProgressBar.style.width = newPercent * (progressHolder.offsetWidth - 2)  + "px";
    updateTimeDisplay();
}

function updateTimeDisplay(){
    currentTimeDisplay.innerHTML = formatTime(video1Q.currentTime);
    if (video1Q.duration) durationDisplay.innerHTML = formatTime(video1Q.duration);
}
...
function initQuery() {
    $.getJSON("/data/newDataQuery.json", function(data) {
        window.dataset = data;
    });
    ...
    var video_length = Math.ceil(video1Q.duration);
    ...
    timeline.on('click', function(properties){
        ...
        // if(seconds > Math.ceil(video1Q.duration)){
        // 	 alert("Incorrect video time");
        // }
        // alert(seconds);
        video1Q.currentTime = seconds;
        video2Q.currentTime = seconds;
...
function colabInBox(text){
    var videoTime = seconds2time(video1Q.currentTime.toFixed(0));
    c = c + 1;
    var textBuffer = c + " . " + text + ". --  " + videoTime;
    showCommenttimeline(video1Q.currentTime, text);
...
function submitCmntFloating() {
    ...
    showCommenttimeline(video1Q.currentTime, document.getElementById("commentsfloat").value);
    var div = document.createElement("div");
    var element = document.getElementById("recordView");
    element.appendChild(div);
    var node = document.createTextNode(textBuffer + "  ");
...
function submitCmnt() {
    ...
    showCommenttimeline(video1Q.currentTime, document.getElementById("comments").value);
    var div = document.createElement("div");
    var element = document.getElementById("recordView");
    element.appendChild(div);
    var node = document.createTextNode(textBuffer + "  ");
...
function pauseAll() {
    ...
    var ratio = video1Q.videoWidth / mycan.width;
    var videoWidth = video1Q.videoWidth / ratio;
    var videoHeight = video1Q.videoHeight / ratio;
    ctx.clearRect(0, 0, 582, 482);
    ctx.globalAlpha = gA;
    ctx.drawImage(video1Q, 0, 0, videoWidth, videoHeight);
    window.count = Math.round(video1Q.currentTime * 4.89);
...
function Distance() {
    ...
    for (var i = 0; i < l; i++) {
        ...
        var a = 462 - window.dataset[window.count].skeletons[i].head.y;
...
function Pause() {
    if (video1Q.paused) {
        video_1.currentTime = video1Q.currentTime;
        video2Q.currentTime = video1Q.currentTime;
        video1Q.play();
        video2Q.play();
        video_1.play();
        var btnlayer = document.getElementById("btnLayer");
    ...
    for(var s=0; s<window.dataset.length; s++){
        ...
        for(var i in window.searchData.orientation){
            ...
            if(pid > l){
                ...
                if(window.dataset[tscount].skeletons[pid-1].head.orientation < (parseFloat(item.orient)+30.00) && window.dataset[tscount].skeletons[pid-1].head.orientation > parseFloat(item.orient)-50.00){
                }
                else{
                    orirange.push(s);
                    break;
        ...
        for(var i=0; i<window.searchData.location.length; i++){
            ...
            for(var j=0; j<item.length; j++){
                ...
                for(var p=0; p<l; p++){
                    if((window.dataset[tscount].skeletons[p].head.x < lox + 60 && window.dataset[tscount].skeletons[p].head.x > lox - 60) && (window.dataset[tscount].skeletons[p].head.y<loy  + 60 && window.dataset[tscount].skeletons[p].head.y > loy - 60)){
                        check = true;
                        break;
                    }
                }
